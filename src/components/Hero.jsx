...
export default function Hero() {
  ...
  return (
    <section id="home" className="relative w-full h-[92vh] overflow-hidden bg-black text-white">
      ...
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(1200px_600px_at_50%_120%,rgba(239,68,68,0.15),transparent)]" />

      {/* Top navigation */}
      <header className={`relative z-10 sticky top-0 transition-all ${elevated ? 'backdrop-blur-md bg-black/40 border-b border-white/10' : 'bg-transparent'}`}>
        <div className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between">
          <a href="#home" className="flex items-center gap-3 group">
            ...
            <div className="leading-tight">
              <p className="font-semibold tracking-wide group-hover:text-white transition-colors">Flames Corporation</p>
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm text-white/80">
            {[
              ...
              { href: '#capabilities', label: 'Layanan' },
              { href: '#contact', label: 'Kontak' },
            ].map((l) => (
              <a key={l.href} href={l.href} className="relative hover:text-white transition">
                <span className="relative z-10">{l.label}</span>
                <span className="pointer-events-none absolute -bottom-1 left-0 h-[2px] w-0 bg-red-500 transition-all group-hover:w-full" />
              </a>
            ))}
        ...
        {open && (
          <div className="md:hidden mx-auto max-w-7xl px-6 pb-4">
            <div className="rounded-lg border border-white/10 bg-black/60 backdrop-blur p-4 flex flex-col gap-3">
      ...
      <div className="relative z-10 h-full">
        <div className="mx-auto max-w-7xl px-6 h-full flex flex-col items-start justify-center">
          ...
          <p className="mt-5 max-w-2xl text-base md:text-lg text-white/80">
            Kami menggabungkan strategi, kreativitas, dan teknologi untuk menghadirkan dampak nyata bagi brand dan masyarakat. Terinspirasi dari standar global, berakar kuat pada pasar Indonesia.
          </p>
          <div className="mt-8 flex flex-col sm:flex-row items-center gap-4">
            <a href="#contact" className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-500 text-white px-6 py-3 rounded-md font-medium transition shadow-[0_0_0_0_rgba(239,68,68,0.6)] hover:shadow-[0_0_30px_6px_rgba(239,68,68,0.25)]">
